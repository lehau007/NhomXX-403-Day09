...
| :--- | :--- | :--- |
| **Hậu** | **Supervisor & Evaluator** | Quản lý luồng chạy chính (`graph.py`), điều phối logic định tuyến, viết code đánh giá (`eval_trace.py`) và báo cáo tổng hợp. |
| **Tú** | **Worker Specialist** | Xây dựng các Worker cốt lõi (`retrieval.py`, `synthesis.py`), định nghĩa hợp đồng dữ liệu (`worker_contracts.yaml`) và so sánh metrics. |
| **Hào** | **Integration & MCP** | Xử lý logic nghiệp vụ phức tạp (`policy_tool.py`), xây dựng hệ thống công cụ ngoài (`mcp_server.py`) và viết kiến trúc hệ thống. |
...
*   **Tú (Comparison & Individual Report):**
...
*   **Hào (System Docs & Individual Report):**
